...
import Data.Map
import Data.ParserInternals.ParserHandle
import Data.ParserInternals.Reciever
import Data.ParserInternals.LinkedData as Linked
...
type RecieverIdentifier = LinkedParsableSegment -> Maybe Reciever

-- | Checks a segment against a chain of identifers and attempts to find a reciever
identifyReciever :: LinkedParsableSegment -> [RecieverIdentifier] -> Maybe Reciever
...
data ParseManager = ParseManager
  { -- | Map of recievers that are currently open
    ...
    parseQueue :: Seq LinkedParsableSegment,
    -- | Parsed data
    parsedData :: Seq LinkedCaptureDict
  }

-- | Parse a ParsableSegment and update parser
parse :: LinkedParsableSegment -> ParseManager -> ParseManager
parse segment parser =
  ...
  where
    -- | Create new parser 
    parseWithIdentifer :: Maybe Reciever -> LinkedParsableSegment -> ParseManager -> ParseManager
      ...
      let oldRecieverMap = activeRecievers parser
       ...
       in ParseManager
            { activeRecievers = updateRecievers newHandle (rid reciever) oldRecieverMap, -- Creates the new reciever map based on the resultcieverMap
              ...
              parsedData = parsed result >< parsedData parser
            }
